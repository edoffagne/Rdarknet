...
pthread_t load_data_in_thread(load_args args)
{
    ...
    *ptr = args;
    if(pthread_create(&thread, 0, load_thread, ptr)) utils_error("Thread creation failed");
    return thread;
}
...
pthread_t load_data(load_args args)
{
    ...
    struct load_args *ptr = calloc(1, sizeof(struct load_args));
    *ptr = args;
    if(pthread_create(&thread, 0, load_threads, ptr)) utils_error("Thread creation failed");
    return thread;
}
