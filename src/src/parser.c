...
local_layer parse_local(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before local layer must output image.");
...
layer parse_deconvolutional(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before deconvolutional layer must output image.");
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before convolutional layer must output image.");
    int batch_normalize = option_find_int_quiet(options, "batch_normalize", 0);
...
crop_layer parse_crop(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before crop layer must output image.");
...
layer parse_reorg(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before reorg layer must output image.");
...
maxpool_layer parse_maxpool(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before maxpool layer must output image.");
...
avgpool_layer parse_avgpool(list *options, size_params params)
{
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) utils_error("Layer before avgpool layer must output image.");
...
route_layer parse_route(list *options, size_params params, network *net)
{
    ...
    int len = strlen(l);
    if(!l) utils_error("Route Layer must specify input layers");
    int n = 1;
    int i;
    for(i = 0; i < len; ++i){
        if (l[i] == ',') ++n;
...
void parse_net_options(list *options, network *net)
{
    ...
    if(!net->inputs && !(net->h && net->w && net->c)) utils_error("No input parameters supplied");
    ...
    if(net->policy == STEP){
        ...
        if(!l || !p) utils_error("STEPS policy must have steps and scales in cfg file");
...
network *parse_network_cfg(char *filename)
{
    ...
    if(!n) utils_error("Config file has no sections");
    ...
    if(!is_network(s)) utils_error("First section must be [net] or [network]");
